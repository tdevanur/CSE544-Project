...
def join(conn, output_dir, table_names, attributes, number_of_samples, p, method):
    ...
    for i in range(len(table_names) - 1):
        if testing_mode == 1:
            explain_query += 'small' + str(int(small_table_probability * 100)) + '_' + table_names[i] + " t" + str(
                i) + ", "
        elif testing_mode == 0:
            explain_query += table_names[i] + " t" + str(i) + ", "
    if testing_mode == 1:
        explain_query += 'small' + str(int(small_table_probability * 100)) + '_' + table_names[
    ...
    output_dic = {'method': str(sampling_method), 'table_names': str(table_names), 'attributes': str(attributes),
                  'sampling_probability': str(round(sampling_probability * small_table_probability, 5)),
